...
1. Use **mod scraper** (Modrinth/CurseForge API) to find mod
2. Use **ferium_manager** to download correct version for MC version + loader
3. Put in appropriate folder (`clientonly/` for client-only deps, `mods/` for server deps)
4. Next manifest fetch automatically includes the new mod
...
- `crash_analyzer.py` - NEW: Crash log analysis (this phase)
...
## Dependency Fetching - CRITICAL

### How to Read Dependencies from JAR Files

DO NOT try to guess the mod's name or slug. Read the dependencies DIRECTLY from the mod's TOML file in the JAR:

```python
import zipfile

def get_mod_dependencies(jar_path: Path) -> Dict[str, List[Dict]]:
    """Read dependencies from a mod JAR file's TOML."""
    deps = {"required": [], "optional": []}
    
    with zipfile.ZipFile(jar_path, 'r') as zf:
        names = zf.namelist()
        
        # Find the mods.toml file
        toml_file = None
        for f in ['META-INF/neoforge.mods.toml', 'META-INF/mods.toml']:
            if f in names:
                toml_file = f
                break
        ...
        toml_data = tomllib.loads(zf.read(toml_file).decode('utf-8'))
        
        # Get ALL dependencies from the [[dependencies]] table
        # The deps are stored as a dict where keys = modIds
            ...
            for dep_mod_id, dep_list in all_deps.items():
                if isinstance(dep_list, list):
                    for dep in dep_list:
                        ...
                        # Skip loader and minecraft deps
                        if dep_id in ("neoforge", "forge", "fabric", "minecraft", "javafml", "fml"):
                            continue
                        ...
                        if side == "CLIENT":
                            deps["optional"].append(dep_info)  # Client-only deps are optional for server
                        else:
                            deps["required"].append(dep_info)
        
        # Also check for side info in the [[mods]] table
        mods_section = toml_data.get("mods", [])
        if mods_section and isinstance(mods_section, list):
            mod_info = mods_section[0]
            mod_side = mod_info.get("side", "BOTH").upper()
            if mod_side == "CLIENT":
                # This entire mod is client-only
                deps["client_only"] = True
...
1. Scan ALL mods in `mods/` folder and extract their dependencies using the method above
2. Build a map of: `mod_id -> {required_deps: [], optional_deps: []}`
3. For each required dep that isn't installed, try to fetch it:
   - Search CurseForge FIRST (better mod coverage, especially for NeoForge)
   - Use scraper with Playwright stealth to bypass Cloudflare
   - Use token matching on search results (NOT slug guessing)
4. Use the "dependents" list to CONFIRM matches - if mod A says it needs mod B, and we find mod B that confirms it has mod A as a dependent, that's a good match

### Token Matching for Search

When searching for mods, tokenize both the query and results:
```python
def tokenize(s):
    return set(re.sub(r'[^a-z0-9]', '', t.lower()) for t in re.findall(r'[A-Za-z]+', s))

# Query: "biomesoplenty" -> {'biomes', 'oplenty'}
# Result title: "Biomes O' Plenty" -> {'biomes', 'o', 'plenty'}
# Match score: 2 tokens match -> good match
```

### Common Issues to Avoid

1. **DO NOT add -lib, -api, -mod suffixes** to mod IDs unless the ID is > 10 chars
2. **DO NOT create slug variations** like `x-a-e-r-o-l-i-b` - use token matching instead
3. **Search CurseForge FIRST** - it has better coverage for NeoForge mods
4. **Use the actual dep list** from the JAR, don't try to guess
