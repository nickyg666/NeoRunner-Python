...
def update_manifest(mods_dir: Path) -> bool:
    """Update manifest.json with current mod list (lightweight)."""
    ...
    try:
        ...
        if clientonly_dir.exists():
            for f in os.listdir(clientonly_dir):
                if f.endswith('.jar') and not f.endswith('.server.jar'):
                    if f not in mods:
                        mods[f] = clientonly_dir / f
        
        # Always update manifest
        manifest = {"files": [{"path": name} for name in sorted(mods.keys())]}
        
        with open(manifest_path, 'w') as f:
            json.dump(manifest, f, indent=2)
        
        log_event("MANIFEST", f"Updated manifest.json ({len(mods)} mods)")
        return True
...
def create_mod_zip(mods_dir: Path) -> Optional[Path]:
    """
...
def conditional_create_mod_zip(mods_dir: Path) -> Optional[Path]:
    """
    ...
    with _zip_creation_lock:
        ...
        # Create the zip
        result = create_mod_zip(mods_dir)
        if result:
...
Write-Host "[3/4] Syncing mods..." -ForegroundColor Yellow
...
$missingCount = $serverCount - $skipped
Write-Host "    Missing: $missingCount" -ForegroundColor Gray

if ($missingCount -gt 0) {
    Write-Host "    Downloading $missingCount missing mods..." -ForegroundColor Cyan
    $zipPath = "$env:TEMP\\neorunner_mods.zip"
    
    try {
        Invoke-WebRequest -Uri "$baseUrl/download/all" -OutFile $zipPath -UseBasicParsing -TimeoutSec 120
...
def generate_bat_script(cfg: ServerConfig) -> str:
    """Generate batch script that downloads only missing mods from server."""
    hostname = _get_server_hostname(cfg)
    http_port = cfg.http_port
