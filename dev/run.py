...
import json, time, threading, logging, hashlib, urllib.request, urllib.error, socket, shutil
...
def check_mc_version_compatibility(world_version, server_version):
    """Check if world MC version is compatible with server MC version.
    ...
    w_major, w_minor, w_patch = parse_ver(world_version)
    s_major, s_minor, s_patch = parse_ver(server_version)
    
    if w_major == s_major and w_minor == s_minor and w_patch == s_patch:
        return True, "exact_match"
    elif w_major == s_major and w_minor == s_minor:
        return True, "minor_match"
    elif w_major != s_major:
        return False, "major_mismatch"
    else:
        return False, "minor_mismatch"

def scan_worlds():
    """Scan for world folders (folders containing level.dat)"""
    c = load_cfg()
    server_mc_version = c.get("mc_version", "1.21.11")
    worlds = []
        ...
        if os.path.isdir(entry_path):
            ...
            if os.path.exists(level_dat):
                try:
                    ...
                    compatible, compat_reason = check_mc_version_compatibility(world_version, server_mc_version)
                    worlds.append({
                        "name": entry,
                        "path": entry_path,
                        "size": sum(
                            ...
                            for f in filenames
                        ) if os.path.isdir(entry_path) else 0,
                        "modified": stat.st_mtime,
                        "mc_version": world_version,
                        "snapshot": version_info.get("snapshot", False),
                        "compatible": compatible,
                        "compatibility_reason": compat_reason
                    })
                except:
                    worlds.append({"name": entry, "path": entry_path, "mc_version": None, "compatible": True})
    return sorted(worlds, key=lambda w: w.get("name", ""))

def switch_world(world_name, force=False):
    """Switch to a different world by updating server.properties.
    
    Args:
        world_name: Name of the world folder
        force: If True, skip version compatibility check
    ...
    if not force:
        c = load_cfg()
        server_mc_version = c.get("mc_version", "1.21.11")
        world_version_info = get_world_version(level_dat)
        world_version = world_version_info.get("version")
        
        if world_version:
            compatible, reason = check_mc_version_compatibility(world_version, server_mc_version)
            if not compatible:
                error_msg = f"Version mismatch: world is MC {world_version}, server is MC {server_mc_version}. Use force=true to override."
                log_event("WORLD_SWITCH", f"Blocked switch to '{world_name}': {error_msg}")
                return False, error_msg
            elif reason == "minor_match":
                log_event("WORLD_SWITCH", f"Warning: world MC {world_version} differs from server MC {server_mc_version} (same major.minor)")
    ...
    log_event("WORLD_SWITCH", f"Switched to world: {world_name} (takes effect on next reboot)")
...
def install_neoforge(mc_version):
    ...
    try:
        ...
        if os.path.exists(neoforge_dir):
            ...
            eula_path = os.path.join(CWD, "eula.txt")
            if not os.path.exists(eula_path):
                with open(eula_path, "w") as f:
                    f.write("eula=true\n")
                log_event("EULA_SETUP", "Created eula.txt with eula=true")
            else:
                # Update existing EULA to ensure it's set to true
                with open(eula_path, "r+") as f:
                    ...
                    if "eula=false" in content.lower():
                        ...
                        f.truncate()
                        log_event("EULA_SETUP", "Updated eula.txt to eula=true")
...
def backup_loader_snapshot(loader_name, mc_version):
    """Create a full snapshot backup of mods and world to {loader}{mcver}/ directory.
    
    This creates a snapshot before switching loaders:
    - {loader}{mcver}/mods/ - copy of all mods
    - {loader}{mcver}/world/ - copy of current world (reads level-name from server.properties)
    
    Returns dict with backup details.
    """
    c = load_cfg()
    mods_dir = os.path.join(CWD, c.get("mods_dir", "mods"))
    clientonly_dir = os.path.join(CWD, c.get("clientonly_dir", "clientonly"))
    
    snapshot_name = f"{loader_name}{mc_version}"
    snapshot_path = os.path.join(CWD, snapshot_name)
    
    backup = {"mods": 0, "clientonly": 0, "world": False, "path": snapshot_path, "world_name": None}
    
    try:
        props = parse_props()
        world_name = props.get("level-name", "world")
        backup["world_name"] = world_name
        world_dir = os.path.join(CWD, world_name)
        
        mods_backup = os.path.join(snapshot_path, "mods")
        world_backup = os.path.join(snapshot_path, "world")
        
        os.makedirs(mods_backup, exist_ok=True)
        ...
        if os.path.isdir(mods_dir):
            for item in os.listdir(mods_dir):
                ...
                if os.path.isfile(src) and item.endswith('.jar'):
                    dst = os.path.join(mods_backup, item)
                    shutil.copy2(src, dst)
                    backup["mods"] += 1
                elif os.path.isdir(src) and item == "clientonly":
                    client_backup = os.path.join(mods_backup, "clientonly")
                    os.makedirs(client_backup, exist_ok=True)
                    for f in os.listdir(src):
                        if f.endswith('.jar'):
                            shutil.copy2(os.path.join(src, f), os.path.join(client_backup, f))
                            backup["clientonly"] += 1
        
        if os.path.isdir(world_dir):
            if os.path.exists(world_backup):
                shutil.rmtree(world_backup)
            shutil.copytree(world_dir, world_backup)
            backup["world"] = True
            log_event("LOADER_BACKUP", f"Backed up world '{world_name}' to {world_backup}")
        
        log_event("LOADER_BACKUP", f"Created snapshot at {snapshot_path}: {backup['mods']} mods + {backup['clientonly']} clientonly + world={backup['world']}")
    except Exception as e:
        log_event("LOADER_BACKUP_ERROR", str(e))
    
    return backup

def archive_loader_mods(loader_name, loader_version, mc_version):
    """Archive mods from current loader to a subfolder"""
    c = load_cfg()
    mods_dir = os.path.join(CWD, c.get("mods_dir", "mods"))
    clientonly_dir = os.path.join(CWD, c.get("clientonly_dir", "clientonly"))
    
    if not loader_version:
        loader_version = get_loader_version(loader_name) or "unknown"
    
    archive_name = f"{loader_name}/{loader_version}/{mc_version}"
    archive_path = os.path.join(CWD, "loader_archive", archive_name)
    
    archived = {"mods": 0, "clientonly": 0, "path": archive_path}
        ...
        if os.path.isdir(mods_dir):
            for item in os.listdir(mods_dir):
                ...
                if os.path.isfile(src) and item.endswith('.jar'):
                    ...
                    client_archive = os.path.join(archive_path, "clientonly")
                    os.makedirs(client_archive, exist_ok=True)
                    for f in os.listdir(src):
                        if f.endswith('.jar'):
                            shutil.move(os.path.join(src, f), os.path.join(client_archive, f))
                            archived["clientonly"] += 1
        
        log_event("LOADER_ARCHIVE", f"Archived {archived['mods']} mods + {archived['clientonly']} clientonly to {archive_path}")
    except Exception as e:
        log_event("LOADER_ARCHIVE_ERROR", str(e))
    
    return archived

def restore_loader_mods(loader_name, loader_version, mc_version):
    """Restore mods from a loader archive"""
    c = load_cfg()
    mods_dir = os.path.join(CWD, c.get("mods_dir", "mods"))
    clientonly_dir = os.path.join(CWD, c.get("clientonly_dir", "clientonly"))
    ...
    archive_path = os.path.join(CWD, "loader_archive", f"{loader_name}/{loader_version}/{mc_version}")
    
    restored = {"mods": 0, "clientonly": 0}
    
    if not os.path.isdir(archive_path):
        return {"error": "Archive not found", "restored": restored}
    ...
    try:
        ...
        for item in os.listdir(archive_path):
            src = os.path.join(archive_path, item)
            if item == "clientonly" and os.path.isdir(src):
                for f in os.listdir(src):
                    if f.endswith('.jar'):
                        shutil.move(os.path.join(src, f), os.path.join(clientonly_dir, f))
                        restored["clientonly"] += 1
            elif item.endswith('.jar'):
                shutil.move(src, os.path.join(mods_dir, item))
                restored["mods"] += 1
        
        shutil.rmtree(archive_path)
        log_event("LOADER_RESTORE", f"Restored {restored['mods']} mods + {restored['clientonly']} clientonly from {archive_path}")
    except Exception as e:
        log_event("LOADER_RESTORE_ERROR", str(e))
    
    return {"restored": restored}

def switch_loader(new_loader, mc_version=None):
    """Switch to a different loader, creating backup snapshot and archiving current mods"""
    c = load_cfg()
    current_loader = c.get("loader", "neoforge").lower()
    current_mc = c.get("mc_version", "1.21.11")
    ...
    if current_loader.lower() == new_loader.lower() and current_mc == target_mc:
        return {"success": False, "error": f"Already using {new_loader} for MC {target_mc}"}
    
    current_version = get_loader_version(current_loader)
    
    backup_result = backup_loader_snapshot(current_loader, current_mc)
    
    archive_result = archive_loader_mods(current_loader, current_version, current_mc)
    
    c["loader"] = new_loader.lower()
    c["mc_version"] = target_mc
    save_cfg(c)
    
    log_event("LOADER_SWITCH", f"Switched from {current_loader} {current_mc} to {new_loader} {target_mc}")
    
    restore_result = restore_loader_mods(new_loader, get_loader_version(new_loader), target_mc)
    
    return {
        "success": True,
        "from_loader": current_loader,
        "from_mc": current_mc,
        "to_loader": new_loader,
        "to_mc": target_mc,
...
def preflight_mod_compatibility_check(mods_dir, cfg):
    ...
    for filename in sorted(os.listdir(mods_dir), key=str.lower):
        ...
        # ── Check 0: Client-only mods should go to clientonly/, not root mods/ ──
        if mod_type == "client":
            clientonly_dir_compat = os.path.join(mods_dir, "clientonly")
            os.makedirs(clientonly_dir_compat, exist_ok=True)
            dest = os.path.join(clientonly_dir_compat, filename)
            if not os.path.exists(dest):
                import shutil as _shutil
                _shutil.move(jar_path, dest)
                log_event("COMPAT_CHECK", f"Moved client-only mod {filename} -> clientonly/")
            else:
                os.remove(jar_path)
                log_event("COMPAT_CHECK", f"Removed duplicate {filename} from mods/ (already in clientonly/)")
            continue
        ...
        if mod_loader and mod_loader.lower() not in compatible_loaders:
            ...
            _qfn = _quarantine_mod(mods_dir, filename, reason)
            if _qfn:
                results["quarantined"].append(_qfn)
            continue
        ...
        if mc_range:
            ...
            if version_ok is False:
                ...
                _qfn2 = _quarantine_mod(mods_dir, filename, reason)
                if _qfn2:
                    results["quarantined"].append(_qfn2)
                continue
    ...
    if FLASK_AVAILABLE:
        def run_flask_app():
            try:
                        ...
                        if fn.endswith('.jar'):
                            ...
                            if os.path.exists(reason_file):
                                ...
                                if "Requires Java" in reason and "server has Java" in reason:
                                    ...
                                    mod_type = classify_mod(src) if os.path.exists(src) else "both"
                                    if mod_type == "client":
                                        dst = os.path.join(clientonly_dir, fn)
                                    else:
                ...
                def api_server_mods():
                    """Get list of server-side mods (root mods folder)"""
                    try:
                        c = load_cfg()
                        mods_dir_path = os.path.join(CWD, c.get("mods_dir", "mods"))
                        mods = []
                        ...
                        if os.path.exists(mods_dir_path):
                            for filename in sorted(os.listdir(mods_dir_path), key=str.lower):
                                if filename.endswith(".jar"):
                                    path = os.path.join(mods_dir_path, filename)
                                    size = os.path.getsize(path)
                                    mod_class = classify_mod(path)
                                    if mod_class in ("server", "both"):
                                        mods.append({
                                            "id": filename,
                                            "name": filename,
                ...
                def api_client_mods():
                    """Get list of client-side mods (clientonly folder)"""
                    try:
                        c = load_cfg()
                        mods_dir_path = os.path.join(CWD, c.get("mods_dir", "mods"))
                        clientonly_dir = os.path.join(mods_dir_path, "clientonly")
                        ...
                        if os.path.exists(clientonly_dir):
                            for filename in sorted(os.listdir(clientonly_dir), key=str.lower):
                                if filename.endswith(".jar"):
                            ...
                            for filename in list(os.listdir(clientonly_dir)):
                                if filename.endswith(".jar"):
                                    ...
                                    if not os.path.exists(dst):
                                        ...
                                        with open(reason_file, "w") as f:
                                            f.write(f"Quarantined: {datetime.now().isoformat()}\n")
                                            f.write(f"Reason: Quarantine all client mods via dashboard\n")
                                            f.write(f"Source: clientonly\n")
                                        
                                        quarantined += 1
                        
                        log_event("QUARANTINE_ALL_CLIENT", f"Quarantined {quarantined} client mods")
                ...
                def api_sort_client_mods():
                    ...
                    try:
                        c = load_cfg()
                        mods_dir_path = os.path.join(CWD, c.get("mods_dir", "mods"))
                        moved = sort_mods_by_type(mods_dir_path)
                        create_mod_zip(mods_dir_path)
...
'''
                ...
                def api_restore_from_quarantine():
                    """Restore ALL quarantined mods back to mods/ or clientonly/ based on classify_mod"""
                    try:
                        import shutil
                        c = load_cfg()
                        m_dir = os.path.join(CWD, c.get("mods_dir", "mods"))
                        clientonly_dir = os.path.join(m_dir, "clientonly")
                        q_dir = os.path.join(m_dir, "quarantine")
                        
                        if not os.path.isdir(q_dir):
                            return jsonify({"success": True, "restored": 0, "message": "No quarantine folder"})
                        
                        os.makedirs(clientonly_dir, exist_ok=True)
                        
                        restored = 0
                        restored_client = 0
                        for filename in list(os.listdir(q_dir)):
                            if not filename.endswith('.jar'):
                                continue
                            ...
                            if mod_type == "client":
                                ...
                                dest = os.path.join(m_dir, filename)
                        ...
                        log_event("QUARANTINE", f"Restored {restored} mods from quarantine ({restored_client} to clientonly/)")
                        
                        create_mod_zip(m_dir)
                        
                        return jsonify({"success": True, "restored": restored, "restored_to_clientonly": restored_client})
                    except Exception as e:
                        return jsonify({"success": False, "error": str(e)}), 400
                ...
                def api_worlds():
                    """Return list of available worlds with version info"""
                    worlds = scan_worlds()
                    props = parse_props()
                    current = props.get("level-name", "world")
                    c = load_cfg()
                    server_mc_version = c.get("mc_version", "1.21.11")
                    return jsonify({"worlds": worlds, "current": current, "server_mc_version": server_mc_version})
                
                @app.route("/api/worlds/scan", methods=["POST"])
                def api_worlds_scan():
                    """Scan for world folders with version info"""
                    worlds = scan_worlds()
                    props = parse_props()
                    current = props.get("level-name", "world")
                    c = load_cfg()
                    server_mc_version = c.get("mc_version", "1.21.11")
                    log_event("WORLD_SCAN", f"Found {len(worlds)} worlds, current: {current}")
                    return jsonify({"success": True, "worlds": worlds, "current": current, "server_mc_version": server_mc_version})
                ...
                def api_worlds_switch():
                    ...
                    try:
                        ...
                        success, message = switch_world(world_name, force=force)
                        if success:
                ...
                @app.route("/api/worlds/check-compatible", methods=["POST"])
                def api_worlds_check_compatible():
                    """Check if a world is compatible with current server MC version"""
                    try:
                        data = request.json
                        world_name = data.get("world", "")
                        if not world_name:
                        ...
                        if not os.path.exists(level_dat):
                            return jsonify({"success": False, "error": f"World '{world_name}' not found"}), 404
                        ...
                        if not world_version:
                            return jsonify({
                                "success": True,
                                "world": world_name,
                                "world_mc_version": None,
                                "server_mc_version": server_mc_version,
                                "compatible": True,
                                "compatibility_reason": "unknown_world_version"
                            })
                        ...
                        return jsonify({
                            "success": True,
                            "world": world_name,
                            "world_mc_version": world_version,
                            "server_mc_version": server_mc_version,
                            "compatible": compatible,
                ...
                def api_mods_analyze():
                    ...
                    try:
                        c = load_cfg()
                        mods_dir = os.path.join(CWD, c.get("mods_dir", "mods"))
                        mc_version = c.get("mc_version", "1.21.11")
                ...
                def api_mods_optimize_load_order():
                    """Optimize mod load order for better mixin compatibility"""
                    try:
                        c = load_cfg()
                        mods_dir = os.path.join(CWD, c.get("mods_dir", "mods"))
                        mc_version = c.get("mc_version", "1.21.11")
                ...
                def api_mods_patch():
                    """Auto-patch mods for mixin compatibility"""
                    try:
                        c = load_cfg()
                        mods_dir = os.path.join(CWD, c.get("mods_dir", "mods"))
                        mc_version = c.get("mc_version", "1.21.11")
                        ...
                        log_event("MOD_PATCH", f"Auto-patched {result.get('patched', 0)} mods")
                        return jsonify(result)
                    except Exception as e:
                        return jsonify({"status": "error", "error": str(e)}), 400
                ...
                def api_mods_compatibility_analysis():
                    """Get full compatibility analysis of installed mods"""
                    try:
                        c = load_cfg()
                        mods_dir = os.path.join(CWD, c.get("mods_dir", "mods"))
                        mc_version = c.get("mc_version", "1.21.11")
                ...
                def api_loaders():
                    ...
                    try:
                        ...
                        c = load_cfg()
                        current = c.get("loader", "neoforge").lower()
                        mc_version = c.get("mc_version", "1.21.11")
                        return jsonify({
                            "loaders": loaders,
                            "current": current,
                ...
                def api_loaders_install():
                    ...
                    try:
                        data = request.json
                        loader = data.get("loader", "").lower()
                        mc_version = data.get("mc_version", "1.21.11")
                        
                        if loader not in ["neoforge", "forge", "fabric"]:
                            return jsonify({"success": False, "error": "Invalid loader"}), 400
                        
                        result = ensure_loader_installed(loader, mc_version)
                        if result:
                            log_event("LOADER_INSTALL", f"Installed {loader} for MC {mc_version}")
                            return jsonify({"success": True, "message": f"{loader} installed successfully"})
                        else:
                            return jsonify({"success": False, "error": f"Failed to install {loader}"}), 400
                    except Exception as e:
                        return jsonify({"success": False, "error": str(e)}), 400
                ...
                def api_loaders_switch():
                    ...
                    try:
                        data = request.json
                        new_loader = data.get("loader", "").lower()
                        mc_version = data.get("mc_version")
                        
                        if new_loader not in ["neoforge", "forge", "fabric"]:
                            return jsonify({"success": False, "error": "Invalid loader"}), 400
                        
                        result = switch_loader(new_loader, mc_version)
                        return jsonify(result)
                    except Exception as e:
                        return jsonify({"success": False, "error": str(e)}), 400
                
                @app.route("/api/loaders/archive", methods=["POST"])
                def api_loaders_archive():
                    """Archive current loader's mods"""
                    try:
                        c = load_cfg()
                        loader = c.get("loader", "neoforge")
                        mc_version = c.get("mc_version", "1.21.11")
                        loader_version = get_loader_version(loader)
                        
                        result = archive_loader_mods(loader, loader_version, mc_version)
                ...
                def api_loaders_snapshots():
                    """List loader+mcver snapshot backups"""
                    try:
                        snapshots = []
                        for entry in os.listdir(CWD):
                            entry_path = os.path.join(CWD, entry)
                            if not os.path.isdir(entry_path):
                                continue
                            if entry in ["mods", "world", "clientonly", "quarantine", "loader_archive",
                                         "libraries", ".config", ".cache", ".local", "logs"]:
                                continue
                            mods_path = os.path.join(entry_path, "mods")
                            world_path = os.path.join(entry_path, "world")
                            
                            if os.path.isdir(mods_path) or os.path.isdir(world_path):
                                mod_count = 0
                                has_world = os.path.isdir(world_path)
                                if os.path.isdir(mods_path):
                                    mod_count = len([f for f in os.listdir(mods_path) if f.endswith('.jar')])
                                    clientonly_path = os.path.join(mods_path, "clientonly")
                                    if os.path.isdir(clientonly_path):
                                        mod_count += len([f for f in os.listdir(clientonly_path) if f.endswith('.jar')])
                                
                                loader_match = re.match(r'^([a-z]+)([\d.]+)$', entry, re.IGNORECASE)
                                if loader_match:
                                    loader = loader_match.group(1).lower()
                                    mc_ver = loader_match.group(2)
                                else:
                                    loader = "unknown"
                                    mc_ver = "unknown"
...
def _quarantine_mod(mods_dir, mod_id_or_slug, reason="unknown"):
    ...
    If the mod is detected as client-only via classify_mod(), moves to clientonly/ instead.
    Returns the filename that was quarantined/moved, or None."""
    import shutil
    quarantine_dir = os.path.join(mods_dir, "quarantine")
    clientonly_dir = os.path.join(mods_dir, "clientonly")
    os.makedirs(quarantine_dir, exist_ok=True)
    ...
    for fn in os.listdir(mods_dir):
        ...
        if slug_lower in fn_norm:
            ...
            mod_type = classify_mod(jar_path) if os.path.exists(jar_path) else "both"
            
            if mod_type == "client":
                target_dir = clientonly_dir
                target_label = "clientonly"
            else:
                target_dir = quarantine_dir
                target_label = "quarantine"
            
            dst = os.path.join(target_dir, fn)
            try:
                if not os.path.exists(dst):
                    shutil.move(src, dst)
                    log_event("QUARANTINE", f"Moved {fn} -> {target_label}/ (reason: {reason}, type: {mod_type})")
                else:
                    os.remove(src)
                    log_event("QUARANTINE", f"Removed duplicate {fn} from mods/ (already in {target_label}/)")
                
                reason_file = os.path.join(target_dir, f"{fn}.reason.txt")
                with open(reason_file, "w") as rf:
                    ...
                    rf.write(f"Mod type: {mod_type}\n")
                    rf.write(f"Target: {target_label}\n")
                
                return fn
            except Exception as e:
                log_event("QUARANTINE", f"Failed to move {fn}: {e}")
                return None
...
def _preflight_dep_check(cfg):
            ...
            if percentage >= 90:
                ...
                if matching_jdk:
                    if _install_jdk(matching_jdk["package"]):
                        ...
                        import shutil
                        clientonly_restore_dir = os.path.join(mods_dir, "clientonly")
                        os.makedirs(clientonly_restore_dir, exist_ok=True)
                        for fn, _ in java_incompatible:
                            ...
                            mod_type = classify_mod(src) if os.path.exists(src) else "both"
                            if mod_type == "client":
                                dst = os.path.join(clientonly_restore_dir, fn)
                            else:
                                dst = os.path.join(mods_dir, fn)
    ...
    if os.path.isdir(quarantine_dir):
        for qfn in list(os.listdir(quarantine_dir)):
            ...
            # Skip mods quarantined for client-only crashes — those should go to clientonly/
            if "client-only" in reason_text.lower() or "noclassdeffounderror" in reason_text.lower():
                qjar_type = classify_mod(qjar) if os.path.exists(qjar) else "both"
                if qjar_type == "client":
                    clientonly_restore = os.path.join(mods_dir, "clientonly")
                    os.makedirs(clientonly_restore, exist_ok=True)
                    dst_client = os.path.join(clientonly_restore, qfn)
                    try:
                        shutil.move(qjar, dst_client)
                        if os.path.exists(reason_file):
                            os.remove(reason_file)
                        log_event("PREFLIGHT", f"Moved client-only mod {qfn} from quarantine to clientonly/")
                    except Exception as e:
                        log_event("PREFLIGHT", f"Failed to move {qfn} to clientonly/: {e}")
                continue
            # Skip corrupt JARs
            ...
            if needed_as_dep or dep_now_available:
                qjar_type = classify_mod(qjar) if os.path.exists(qjar) else "both"
                if qjar_type == "client":
                    clientonly_restore2 = os.path.join(mods_dir, "clientonly")
                    os.makedirs(clientonly_restore2, exist_ok=True)
                    dst = os.path.join(clientonly_restore2, qfn)
                else:
                ...
                try:
                    ...
                    if os.path.exists(reason_file):
                        os.remove(reason_file)
                    log_event("PREFLIGHT", f"Restored {qfn} from quarantine to {'clientonly/' if qjar_type == 'client' else 'mods/'} (reason was: {reason_text.split(chr(10))[1].strip() if chr(10) in reason_text else 'unknown'})")
            ...
            if os.path.isdir(quarantine_dir):
                for qfn in list(os.listdir(quarantine_dir)):
                    ...
                    if q_mod.lower() == dep_id:
                        qjar_type = classify_mod(qjar) if os.path.exists(qjar) else "both"
                        if qjar_type == "client":
                            clientonly_trans = os.path.join(mods_dir, "clientonly")
                            os.makedirs(clientonly_trans, exist_ok=True)
                            dst = os.path.join(clientonly_trans, qfn)
                        else:
                            dst = os.path.join(mods_dir, qfn)
                        try:
                            shutil.move(qjar, dst)
                            ...
                            if os.path.exists(reason_file):
                                os.remove(reason_file)
                            log_event("PREFLIGHT", f"Restored {qfn} from quarantine to {'clientonly/' if qjar_type == 'client' else 'mods/'} (required by {', '.join(sorted(requesters)[:2])})")
