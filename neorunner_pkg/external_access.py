...
def cloudflared_config(cfg: ServerConfig, domain: str) -> str:
    ...
    Cloudflare ingress rules match on *hostname* (and path for HTTP) only; a
    tunnel cannot route the same hostname to two services by protocol. So the
    tunnel carries HTTPS only — the dashboard (which serves /admin, /download
    and a root page that hands Minecraft clients the join address). The
    Minecraft game port (``cfg.mc_port``) is reached directly, not through the
    tunnel, so no TCP ingress rule is emitted here.
    """
    return f"""\
