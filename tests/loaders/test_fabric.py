...
def _make_loader():
    ...
    return FabricLoader(cfg, Path("/tmp"))


class TestFabricCrashDetection:
    def test_missing_dependency_detection(self):
        """Fabric-style dependency errors."""
        loader = _make_loader()
        for sample in [
            "requires any version of architectury",
            "unmet dependency: fabric-api",
            "missing mod: sodium",
            "resolution failed for mixinextras",
        ]:
            result = loader.detect_crash_reason(sample)
            assert result.get("type") == "missing_dep", sample
            assert result.get("dep") in ("architectury", "fabric-api", "sodium", "mixinextras")
    ...
    def test_mod_loading_error_detection(self):
        """Fabric loader/modloading errors."""
        loader = _make_loader()
        result = loader.detect_crash_reason("fabric-loader: error while loading mod")
    ...
    def test_validate_jvm_args(self, tmp_path):
        """JVM args validation for fabric."""
        loader = _make_loader()
    ...
    def test_build_java_command(self, tmp_path, monkeypatch):
        ...
        loader = _make_loader()
        loader.cwd = tmp_path
        (tmp_path / "fabric.jar").write_bytes(b"x")
        monkeypatch.setattr(
            "neorunner_pkg.loaders.fabric.subprocess.run",
            lambda *a, **k: None,
        )
        cmd = loader.build_java_command()
        assert cmd[0] == "java"
        assert cmd[1] == "@user_jvm_args.txt"
        assert cmd[3].endswith("fabric.jar")

    def test_get_loader_display_name(self):
        """Display name for fabric."""
        loader = _make_loader()
