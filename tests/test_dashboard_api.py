...
class TestDashboardAPIRoutes:
    ...
    @pytest.fixture
    def client(self):
        """Create test client with admin Basic Auth."""
        import base64

        from neorunner_pkg.dashboard import app
        app.config['TESTING'] = True
        app.config['AUTH_HEADER_TESTING'] = 'Basic ' + base64.b64encode(b'mc:123').decode()
        with app.test_client() as client:
            client.environ_base['HTTP_AUTHORIZATION'] = app.config['AUTH_HEADER_TESTING']
            yield client
...
class TestLoaderSwitch:
    ...
    def client(self):
        ...
        app.config['TESTING'] = True
        with app.test_client() as client:
            client.environ_base['HTTP_AUTHORIZATION'] = 'Basic ' + base64.b64encode(b'mc:123').decode()
            yield client
